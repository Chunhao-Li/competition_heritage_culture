...
task_ner_labels = {
    ...
    'my':['尺寸', '人物', '遗址文化/文物藏品/人物','建筑','文物藏品','遗址文化/单位机构','地点','面积',
         '单位机构','遗址文化/文物藏品','保护级别','书籍文献','历史时代','文化','时间','遗址文化','重量']
}

task_rel_labels = {
    'scierc': ['PART-OF', 'USED-FOR', 'FEATURE-OF', 'CONJUNCTION', 'EVALUATE-FOR', 'HYPONYM-OF', 'COMPARE'],
    'my': ['相关文物', '所在地', '所属时代', '文物尺寸', '面积大小', '发现时间', '保存于', '建设时间', '文保级别', '相关建筑', '相关人物', '相关单位', '馆藏物品', '文化类型', '成立时间', '文化组成', '相关文献', '文物重量', '馆长']
}
...
class ACEDataset(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        if args.data_dir.find('my') != -1:
            self.ner_label_list = ['NIL', '尺寸',
                                   '人物',
                                   '遗址文化/文物藏品/人物',
                                   '建筑',
                                   '文物藏品',
                                   ...
                                   '时间',
                                   '遗址文化','重量']
            label_list = ['相关文物', '所在地', '所属时代', '文物尺寸', '面积大小', '发现时间', '保存于', '建设时间', '文保级别', '相关建筑', '相关人物', '相关单位', '馆藏物品', '文化类型', '成立时间', '文化组成', '相关文献', '文物重量', '馆长']
            self.sym_labels = ['NIL']
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                # left_context_length = right_context_length = 0 # do not use cross-sentence
                ...
                for x in sentence_relations:
                    ...
                    #     print(((l_idx, n), (x[0], x[1]), (x[2], x[3]), x[4]))
                ...
                for sub in entities:
                    ...
                    if sub[0] < 10000:
                        ...
                        sub_tokens = target_tokens + ['[unused1]', '[unused2]']
                        sub_label = -1
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    if do_test:
        ...
        logger.info("Finish test prediction!!")
    ...
    #
    # tot_recall = len(golden_labels)
    # p = cor / tot_pred if tot_pred > 0 else 0
    # r = cor / tot_recall
    # f1 = 2 * (p * r) / (p + r) if cor > 0 else 0.0
    # # assert (tot_recall == len(golden_labels))  # TODO repeated relations
    # print(tot_recall == len(golden_labels))
    #
    # tot_recall = len(golden_labels_withner)
    ...
    # print(tot_recall == len(golden_labels_withner))
    ...
    # results = {'f1': f1, 'precision': p, 'recall': r, 'f1_with_ner': f1_with_ner, 'ner_f1': ner_f1}
    #
...
def main():
    ...
    parser.add_argument('--eval_unidirect', action='store_true') # ablation study
    ...
    if args.do_train:
        subject_id = tokenizer.encode('su', add_special_tokens=False)
